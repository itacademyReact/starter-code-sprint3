...
// He añadido más productos y las ofertas al array products 
var products = [
    {
        name: 'cooking oil',
        price: 10.5,
...
// He añadido 3 categorias más a subtotal 
var subtotal = {
    grocery: {
        value: 0,
        discount: 0
...
function addToCartList (id) {
    ...
    for (let product of products) {
        if (product.name.toLowerCase() == id.toLowerCase()) {
            ...
            console.log('cartList product added', cartList);
...
function generateCart () {
    // Genera cart a partir de carList comprobando que no haya productos repetidos 
    // Esta funcion la sustituye addToCart por eso esta comentada y no la uso 

    // for (let item of cart) {
    //     if (item.name == cartList.at(-1).name) {
    //         item.quantity++
    //         item.subtotal = item.quantity * cartList.at(-1).price
    ...
    // }
    // let newItem = {
    //     name: cartList.at(-1).name,
    //     price: cartList.at(-1).price,
    //     type: cartList.at(-1).type,
    //     quantity: 1,
    //     subtotal: 1 * cartList.at(-1).price,
    //     subtotalWithDiscount: 1 * cartList.at(-1).price
    // }
    // cart.push(newItem)
    // console.log('cart item added', cart);
}
...
function removeFromCartList (name) {
    for (let product of cartList) {
        if (product.name === name) {
            ...
            console.log('cartList product removed', cartList);
            console.log('subtotal', subtotal)
            console.log('total', total);
            return
        }
...
//Eventos para addToCartList que no uso porque los sustituye addToCart
// grocery.addEventListener('click', (e) => {
//     if (e.target.classList.contains('btn-primary')) {
//         addToCartList(e.target.previousElementSibling.previousElementSibling.innerText)
//     }
// });
